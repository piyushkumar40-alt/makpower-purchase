...
async function setupPgDatabase() {
  ...
  try {
    ...
    // Auto-sync 4-Month Party Category Sales on startup
    try {
      await syncPartyCategoryMonthlySales();
    } catch (syncErr) {
      console.warn("Party category sales initial sync notice:", syncErr.message);
    }
  } catch (err) {
        ...
        if (myParties.length > 0) {
          ...
          const imsPartyQuery = `
            ...
            WHERE ("partyName" IS NOT NULL AND TRIM("partyName") <> '') 
              AND ("movementType" = 'OUT' OR "movementType" IS NULL OR "movementType" = '')
